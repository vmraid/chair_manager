...
def run_command(commands, doctype, key, cwd="..", docname=" ", after_command=None):
	...
	start_time = vmraid.utils.time.time()
	console_dump = ""
	...
	try:
		for command in commands:
			...
			for c in iter(lambda: safe_decode(terminal.stdout.read(1)), ""):
				vmraid.publish_realtime(key, c, user=vmraid.session.user)
				console_dump += c
		if terminal.wait():
			_close_the_doc(
				start_time, key, console_dump, status="Failed", user=vmraid.session.user
			)
		else:
			_close_the_doc(
				start_time, key, console_dump, status="Success", user=vmraid.session.user
			)
		...
		vmraid.db.commit()
		# hack: vmraid.db.commit() to make sure the log created is robust,
		# and the _refresh throws an error if the doc is deleted
		vmraid.enqueue(
			"chair_manager.chair_manager.utils._refresh",
			doctype=doctype,
			docname=docname,
			commands=commands,
		)
...
def _close_the_doc(start_time, key, console_dump, status, user):
	time_taken = vmraid.utils.time.time() - start_time
	final_console_dump = ""
	...
	cmd = vmraid.get_doc("Chair Manager Command", key)
...
def _refresh(doctype, docname, commands):
	vmraid.get_doc(doctype, docname).run_method("after_command", commands=commands)


@vmraid.whitelist()
def verify_whitelisted_call():
	if "chair_manager" not in vmraid.get_installed_apps():
		raise ValueError("This site does not have chair manager installed.")
