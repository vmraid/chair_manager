...
vmraid.ui.form.on('App', {
	onload: function(frm) {
		if (frm.doc.__islocal != 1) frm.save();
		vmraid.realtime.on("Chair-Manager:reload-page", () => frm.reload_doc());
	},
	refresh: function(frm) {
		...
		if (frm.doc.is_git_repo != true) {
			...
			frm.add_custom_button(__('Pull & Rebase'), function(){
				vmraid.call({
					method: 'chair_manager.chair_manager.doctype.app.app.get_remotes',
					args: {
			...
			frm.add_custom_button(__('Switch Branch'), function(){
				vmraid.call({
					method: 'chair_manager.chair_manager.doctype.app.app.get_branches',
					args: {
						doctype: frm.doctype,
						docname: frm.doc.name,
					...
					btn: this,
					callback: function(r) {
						if(!r.message) vmraid.msgprint('This app has just one branch');
						else {
							var dialog = new vmraid.ui.Dialog({
								title: 'Select Branch',
								fields: [
