...
import vmraid
from chair_manager.chair_manager.utils import verify_whitelisted_call
	...
	def validate(self):
		if self.get("__islocal"):
			if self.developer_flag == 0:
				vmraid.throw("If you want to create a backup, then goto Sites")
...
def get_restore_options(doctype, docname):
	...
	return [x["name"] for x in vmraid.get_all("Site")]
...
def restore_backup(
	...
	verify_whitelisted_call()
	backup = vmraid.get_doc("Site Backup", docname)
	commands = []
	...
	if on_a_new_site == "1":
		...
		commands.append(
			"chair new-site {site_name} {password_suffix}".format(
				site_name=site_name, password_suffix=password_suffix
			)
		)
	command = "chair --site {site_name} --force restore {backup_file_path}_database.sql".format(
