...
vmraid.ui.form.on('Site Backup', {
	refresh: function(frm) {
		frm.add_custom_button(__('Restore'), function(){
			vmraid.call({
				method: 'chair_manager.chair_manager.doctype.site_backup.site_backup.get_restore_options',
				args: {
					doctype: frm.doctype,
				...
				btn: this,
				callback: function(r) {
					var dialog_data = new vmraid.ui.Dialog({
					...
					dialog_data.set_primary_action(__("Verify"), () => {
						vmraid.call({
							...
							btn: this,
							callback: function(r){
								var verification_dialog = new vmraid.ui.Dialog({
									title: 'Are you sure?',
									fields: [
									...
									vmraid.call({
										...
										callback: function(r){
											if (r.message == "console"){
												...
												console_dialog(key);
												vmraid.call({
													method: 'chair_manager.chair_manager.doctype.site_backup.site_backup.restore_backup',
													args: {
														doctype: frm.doctype,
														docname: frm.doc.name,
