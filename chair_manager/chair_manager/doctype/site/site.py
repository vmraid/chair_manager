...
class Site(Document):
	...
	def after_command(self, commands=None):
		vmraid.publish_realtime("Chair-Manager:reload-page")
	...
	def get_installed_apps(self):
		...
		if retry == 3 and self.site_name not in all_sites:
			...
			list_apps = check_output(
				shlex.split("chair --site {site_name} list-apps".format(site_name=self.site_name)),
				cwd="..",
			)

		if "vmraid" not in safe_decode(list_apps):
			list_apps = "vmraid"
	...
	def sync_site_config(self):
		if os.path.isfile(self.site_name + "/site_config.json"):
			...
			vmraid.throw("The site you're trying to access doesn't actually exist.")

	@vmraid.whitelist()
	def create_alias(self, key, alias):
		files = check_output("ls")
		if alias in files:
			vmraid.throw("Sitename already exists")
		else:
	...
	def console_command(
		...
		commands = {
			"migrate": ["chair --site {site_name} migrate".format(site_name=self.name)],
			"create-alias": [
				"ln -s {site_abspath} sites/{alias}".format(site_abspath=site_abspath, alias=alias)
			],
			"delete-alias": ["rm sites/{alias}".format(alias=alias)],
			"backup": [
				"chair --site {site_name} backup --with-files".format(site_name=self.name)
			],
			"reinstall": [
				"chair --site {site_name} reinstall --yes --admin-password {admin_password}".format(
					site_name=self.name, admin_password=admin_password
				)
			],
			"install_app": [
				"chair --site {site_name} install-app {app_name}".format(
					site_name=self.name, app_name=app_name
				)
			],
			"uninstall_app": [
				"chair --site {site_name} uninstall-app {app_name} --yes".format(
					site_name=self.name, app_name=app_name
				)
			],
			"drop_site": [
				"chair drop-site {site_name} --root-password {mysql_password}".format(
...
def get_installable_apps(doctype, docname):
	...
	with open(app_list_file, "r") as f:
		apps = f.read().split("\n")
	installed_apps = vmraid.get_doc(doctype, docname).app_list.split("\n")
...
def get_removable_apps(doctype, docname):
	verify_whitelisted_call()
	removable_apps = vmraid.get_doc(doctype, docname).app_list.split("\n")
	removable_apps.remove("vmraid")
...
def verify_password(site_name, mysql_password):
	...
	try:
		db = pymysql.connect(
			host=vmraid.conf.db_host or "localhost", user="root", passwd=mysql_password
		)
		db.close()
	except Exception as e:
		print(e)
		vmraid.throw("MySQL password is incorrect")
	return "console"


@vmraid.whitelist()
def create_site(site_name, install_erpadda, mysql_password, admin_password, key):
	verify_whitelisted_call()
	commands = [
		"chair new-site --mariadb-root-password {mysql_password} --admin-password {admin_password} --no-mariadb-socket {site_name}".format(
			site_name=site_name, admin_password=admin_password, mysql_password=mysql_password
		)
	]
	if install_erpadda == "true":
		with open("apps.txt", "r") as f:
			app_list = f.read()
		if "erpadda" not in app_list:
			commands.append("chair get-app erpadda")
		commands.append(
			"chair --site {site_name} install-app erpadda".format(site_name=site_name)
		)
	vmraid.enqueue(
		"chair_manager.chair_manager.utils.run_command",
		commands=commands,
		doctype="Chair Settings",
		key=key,
	)
	all_sites = safe_decode(check_output("ls")).strip("\n").split("\n")
	while site_name not in all_sites:
		...
		all_sites = safe_decode(check_output("ls")).strip("\n").split("\n")
	doc = vmraid.get_doc(
		{"doctype": "Site", "site_name": site_name, "app_list": "vmraid", "developer_flag": 1}
	)
	doc.insert()
	vmraid.db.commit()
