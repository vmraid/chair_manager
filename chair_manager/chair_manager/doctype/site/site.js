...
vmraid.ui.form.on('Site', {
	onload: function(frm) {
		if (frm.doc.__islocal != 1) {
			frm.save();
			frm.call("update_app_alias");
		}
		vmraid.realtime.on("Chair-Manager:reload-page", () => {
			frm.reload_doc();
		});
	},
	...
	refresh: function(frm) {
		...
		frm.add_custom_button(__('Install App'), function(){
			vmraid.call({
				method: 'chair_manager.chair_manager.doctype.site.site.get_installable_apps',
				args: {
					doctype: frm.doctype,
					docname: frm.doc.name
				},
		...
		frm.add_custom_button(__('Uninstall App'), function(){
			vmraid.call({
				method: 'chair_manager.chair_manager.doctype.site.site.get_removable_apps',
				args: {
					doctype: frm.doctype,
					docname: frm.doc.name
				},
		...
		frm.add_custom_button(__('View site'), () => {
			vmraid.db.get_value('Chair Settings', 'Chair Settings', 'webserver_port',
				(r) => {
